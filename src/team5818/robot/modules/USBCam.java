...
public class USBCam {
    ...
    private static String ATTR_WB_VALUE =
    ...
    private Pattern m_reMode = Pattern.compile(
            "(?<width>[0-9]+)\\s*x\\s*(?<height>[0-9]+)\\s+(?<format>.*?)\\s+(?<fps>[0-9.]+)\\s*fps");
    ...
    public synchronized void updateSettings() {
        ...
        NIVision.dxEnumerateVideoModesResult enumerated =
                NIVision.IMAQdxEnumerateVideoModes(m_id);
        ...
        if (foundMode != null) {
            ...
            if (foundMode.Value != enumerated.currentMode)
                NIVision.IMAQdxSetAttributeU32(m_id, ATTR_VIDEO_MODE,
                        foundMode.Value);
        }
        ...
        if (m_whiteBalance == "auto")
            ...
            if (m_whiteBalanceValue != -1)
                NIVision.IMAQdxSetAttributeI64(m_id, ATTR_WB_VALUE,
                        m_whiteBalanceValue);
        }

        // Exposure
        if (m_exposure == "auto")
            NIVision.IMAQdxSetAttributeString(m_id, ATTR_EX_MODE,
                    "AutoAperaturePriority");
        else {
            NIVision.IMAQdxSetAttributeString(m_id, ATTR_EX_MODE, "Manual");
            if (m_exposureValue != -1) {
                long minv = NIVision.IMAQdxGetAttributeMinimumI64(m_id,
                        ATTR_EX_VALUE);
                long maxv = NIVision.IMAQdxGetAttributeMaximumI64(m_id,
                        ATTR_EX_VALUE);
                long val = m_exposureValue;
        ...
        long val = minv + (long) (((double) (maxv - minv))
    ...
    public synchronized void getImageData(ByteBuffer data) {
        ...
        NIVision.IMAQdxGetImageData(m_id, data,
                NIVision.IMAQdxBufferNumberMode.BufferNumberModeLast, 0);
        ...
        while (true) {
            try {
                ...
                throw new VisionException(
                        "invalid image: could not find jpeg end "
                                + ex.getMessage());
            }
        }
    }
