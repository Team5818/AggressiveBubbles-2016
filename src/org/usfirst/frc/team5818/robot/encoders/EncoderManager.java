...
public interface EncoderManager {
    ...
    /**
     * Sets the distance for the encoder to move. If used, this assumes that
     * the EncoderManager will regularly be fed getPowerAccordingToDistance(time) in order
     * to keep updated.
    ...
    /**
     * After having called setDriveDistance(), this function must be called regularly and the
     * power returned should be fed into both left and right motors.
     * @param time - the time in seconds since the last call to setDriveDistance. Critical for
     ...
     * setDriveDistance() or setDriveDistance() has never been called), returns 0.0;
     */
    double getPowerAccordingToDistance(double time);

    /**
     * 
     * @return the distance remaining to the current target as set in setDriveDistance(). If
     * this EncoderManager has no target, ie resetDistance() has been called more recently than
     * the last setDriveDistance() or setDriveDistance() has not been called at all, this returns 0.0
     */
    double getDistanceNotCovered();

    /**
     * 
     * @return the distance originally set in setDriveDistance(), or 0.0 if resetDistance() has been
     * called more recently than the last setDriveDistance() or setDriveDistance() has never been
     * called.
     */
    ...
    /**
     * Resets the target, stopping any in-progress tracking and so forth. Will not be triggered
     * by default, requires somebody to call this.
     */
    void resetDistance();

    /**
     * Shorthand for isDistanceCovered(errorMargin) with errorMargin predefined to some constant we select based on tests.
